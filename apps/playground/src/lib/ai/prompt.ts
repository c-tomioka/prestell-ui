...
function projectPrompt(
	...
	const sections = [
		...
		"- Pages are ALWAYS `src/pages/<name>.astro` files that render a full document through the layout (`src/layouts/Layout.astro`); components render fragments and give every prop a default value. A landing page, a home page, or any new page means editing or adding a `src/pages/*.astro` file.",
		"- Never create `.html` files. `public/` holds static assets only (images, `robots.txt`, a manifest); pages never go there and nothing in `public/` is rendered by the preview.",
		"",
		"## Hard constraints of the live preview (violations will not render)",
