// Retry / timeout policy for the chat request, shared by `/api/chat` (server
// mode) and the browser-side direct transport. Pure helpers and constants so
// the numbers live in one place and the retry loop is unit-testable.

/** Max tool-calling rounds per reply (search → answer). */
export const LLM_MAX_STEPS = 5;
/**
