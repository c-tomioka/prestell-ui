...
## 現在のフェーズ
Phase 3 完了（2026-09-07 に public 化、`v0.1.0`）。Phase 4「静的ホスト版（BYOK）」に着手中: AI direct モード（ブラウザ → LLM、`src/lib/ai/direct/*`）、AI Gateway / Workers AI の CORS 検証（direct 非対応）、Astro Docs MCP の中継 Worker（`apps/playground/relay/`）は完了。次は別オリジンのプレビュー sandbox、静的ビルド構成、BYOK の説明 UI。その後に Phase 5「サイトビルダー（複数ファイル・Astro プロジェクト出力）」、Phase 6「Astro フル機能」が続く（方針は `ROADMAP.md` と `ARCHITECTURE.md` の「Phase 5 の構成」に記録済み。Phase 4 完了までは着手しない）。公開範囲と境界は `OSS_SCOPE.md`、フェーズの全体像は `ROADMAP.md` を参照。Phase 7〜8（SaaS・課金）には着手しない。
