...
## 現在のフェーズ
Phase 3 完了（2026-09-07 に public 化、`v0.1.0`）。Phase 4「静的ホスト版（BYOK）」は 2026-09-08 に完了: AI direct モード（ブラウザ → LLM、`src/lib/ai/direct/*`）、AI Gateway / Workers AI の CORS 検証（direct 非対応）、Astro Docs MCP の中継 Worker（`apps/playground/relay/`）、別オリジンのプレビュー sandbox（`src/pages/preview/`、`PUBLIC_PREVIEW_ORIGIN`）、静的ビルド構成（`pnpm build:static`、`wrangler.static.jsonc`、`PUBLIC_AI_CONNECTIONS`）、BYOK の説明 UI（`DirectModeHelp.svelte`）、i18n（`src/lib/i18n/`、EN / 日本語）まで完了。UI 文言は直書きせず `src/lib/i18n/en.ts` にキーを追加して `ja.ts` にも訳を入れる。Phase 5「サイトビルダー（複数ファイル・Astro プロジェクト出力）」に着手中（2026-09-08）: 複数モジュールのプレビュー（`src/lib/preview-graph.ts`、browser = Blob URL グラフ / server = Worker Loader `modules`）、プロジェクトモデル v2（`files` + `entry` + `mode`、v1 は読み出し時に移行）、モードプリセット（`src/lib/projects/presets.ts`）、ファイルツリー（`FileTree.svelte`）、ページ切替まで完了。残り: AI の多ファイル生成、画像アップロード（`public/` の Blob と `img-src`）、ZIP / ディレクトリ書き出し、Share URL の扱い、評価ハーネスの多ファイル対応。その後 Phase 6「Astro フル機能」（`ROADMAP.md`、`ARCHITECTURE.md` の「Phase 5 の構成」）。公開範囲と境界は `OSS_SCOPE.md`、フェーズの全体像は `ROADMAP.md` を参照。Phase 7〜8（SaaS・課金）には着手しない。
