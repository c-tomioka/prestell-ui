...
**2026-09-07 に公開（`v0.1.0`）**。[ロードマップ](./docs/ROADMAP.md) の Phase 3 は完了し、次は静的ホスト版（BYOK）です。メンテナーが日常的に単一コンポーネントの生成に使っています。プレビューは自己完結した 1 つの `.astro` コンポーネントが対象で、`import`・フレームワークコンポーネント・`client:*`・外部スクリプトには未対応です。静的ホスト版（BYOK）と SaaS 版は後のフェーズで、SaaS 専用コードはこのリポジトリの外にあります（[docs/OSS_SCOPE.md](./docs/OSS_SCOPE.md)）。
