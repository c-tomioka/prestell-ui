...
- モードとファイル（Phase 5）: `presets.ts` が New project の初期ファイル（Component = `Component.astro` 1 本、Page = `src/pages/index.astro` + `src/layouts/Layout.astro` + `src/styles/global.css`、Site = Page + `src/pages/about.astro` + `src/components/Header.astro` / `Footer.astro`）と Component → Page の昇格（`promoteToPage`: コンポーネントを `src/components/<Name>.astro` に移し、レイアウト付きの index から表示）を持つ。`files.ts` がパス規則（`validateFilePath`: 英数字・`.`・`-`・`_` の相対パス、Page / Site は `src/` か `public/` 配下、`.astro` は `src/` 配下、テキスト拡張子のみ）、ツリー構築、入口候補（`src/pages/**` を先頭）、追加・改名・削除の純関数を持つ。UI は `FileTree.svelte`（画面左端、Page / Site で表示、折りたたみ可）とエディタ上のタブ、Preview コントロールの入口 select（ページ切替）。Component モードは従来どおりファイル名入力 1 つでツリーなし。画像はツリーの「画像をアップロード」で `public/images/` に Blob として入り（1 ファイル 2 MB、1 プロジェクト 20 MB。`files.ts` の `checkUploadSize` / `uploadPathFor`）、エディタ側にはプレビューと参照用 URL（`/images/x.png`）を表示する。プレビューでは `preview-assets.ts` が `public/` の参照を `data:` URL に書き換える（`PREVIEW_RENDERING.md`）。チャットは Page / Site では多ファイル契約（下記）で動き、Component では従来どおり単一コンポーネントを編集する。
...
- 多ファイル生成（Phase 5）: Page / Site では要求に `project`（`src/lib/ai/project-context.ts` の `ProjectContext`: テキストファイルの本文と `public/` バイナリの一覧）を付け、`buildSystemPrompt` が「変更・追加するファイルごとに ```astro path=… のコードブロック 1 つ」という契約に切り替わる（全ファイル 40k 文字を超える分は、開いているファイルとその import 先、入口を優先し、残りはパスだけ列挙）。返答は `extractProposalFiles` でパスごとに取り出し、`validateProjectProposal`（パス規則 → 各 `.astro` のコンパイルとプレビュー規則 → 入口ページのグラフ構築）で検証してから `Playground` が一括適用する。fix ループの依頼文も多ファイル用。削除・改名は契約外（プロースで伝えさせる）。ページは必ず `src/pages/*.astro`: `.html` の提案と `public/` だけを触る提案は検証で拒否し、fix ループで Astro 側へ誘導する。テンプレートに `project` カテゴリ（Page / Site でのみ表示）。`/api/chat` と direct transport の両方が `project` を渡し、評価ハーネスは `PROJECT_CASES` で同じ検証を行う。
