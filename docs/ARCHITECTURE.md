...
- モードとファイル（Phase 5）: `presets.ts` が New project の初期ファイル（Component = `Component.astro` 1 本、Page = `src/pages/index.astro` + `src/layouts/Layout.astro` + `src/styles/global.css`、Site = Page + `src/pages/about.astro` + `src/components/Header.astro` / `Footer.astro`）と Component → Page の昇格（`promoteToPage`: コンポーネントを `src/components/<Name>.astro` に移し、レイアウト付きの index から表示）を持つ。`files.ts` がパス規則（`validateFilePath`: 英数字・`.`・`-`・`_` の相対パス、Page / Site は `src/` か `public/` 配下、`.astro` は `src/` 配下、テキスト拡張子のみ）、ツリー構築、入口候補（`src/pages/**` を先頭）、追加・改名・削除の純関数を持つ。UI は `FileTree.svelte`（画面左端、Page / Site で表示、折りたたみ可）とエディタ上のタブ、Preview コントロールの入口 select（ページ切替）。Component モードは従来どおりファイル名入力 1 つでツリーなし。画像はツリーの「画像をアップロード」で `public/images/` に Blob として入り（1 ファイル 2 MB、1 プロジェクト 20 MB。`files.ts` の `checkUploadSize` / `uploadPathFor`）、エディタ側にはプレビューと参照用 URL（`/images/x.png`）を表示する。プレビューでは `preview-assets.ts` が `public/` の参照を `data:` URL に書き換える（`PREVIEW_RENDERING.md`）。チャットはアクティブなファイルを単一コンポーネントとして編集する（多ファイル生成は後続。Page / Site では相対 import を含む提案も `validateProposal` の `checkImport` で受け付ける）。
