...
- 複数ファイル（Phase 5）: ＋ → Site で新規作成すると左端にツリー（`src/components`、`src/layouts`、`src/pages`、`src/styles`）が出て、プレビューが `index.astro` をレイアウト・Header・Footer 込みで描画すること。Preview の「ページ」で `about.astro` に切り替わること。`Header.astro` を編集すると入口ページが再描画されること。ツリーの「新しいファイル」で `src/components/Card.astro` を追加するとテンプレートが入りタブが開くこと、同名で「既にあります」、`public/x.astro` で「src/ の下に」の警告が出ること、改名でパスを変えると移動になること、入口ファイルの削除が拒否されること。Component プロジェクトの「Page プロジェクトに変換」で `src/components/<Name>.astro` + `Layout.astro` + `index.astro` になり描画されること。リロード後に Site プロジェクトのファイル・入口が復元され、Phase 2 で作った旧プロジェクト（IndexedDB の `schemaVersion: 1`）が Component モードとしてそのまま開けること。Component モードではツリーが出ず、Share が有効なこと（Page / Site では無効でツールチップ表示）
- 書き出し（Phase 5）: Page / Site プロジェクトでツールバーの Export（ダウンロードアイコン）から「ZIP をダウンロード」を選ぶと `<slug>.zip` が落ち、展開して `npm install && npm run build` が通り `dist/index.html` にプレビューと同じ構造の HTML が出ること（scoped style は Astro の既定 `data-astro-cid-*` 属性になる）。Chromium では「フォルダーに書き込む…」でフォルダー選択ダイアログが出ること。Component プロジェクトでは Export が出ないこと
- 画像（Phase 5）: Site プロジェクトでツリーの「画像をアップロード」から PNG を選ぶと `public/images/<slug>.png` が増え、エディタ側にプレビューと `/images/<slug>.png` の案内が出ること。`index.astro` に `<img src="/images/<slug>.png">` と `<img src="https://placehold.co/80">` を書くと両方プレビューに表示されること（srcdoc の CSP は `img-src https: data: blob:`、`http:` は表示されない）。3 MB の画像は「2.0 MB を超えています」で拒否されること。ZIP 書き出しに `public/images/` が含まれること
- AI の多ファイル生成（Phase 5）: Site プロジェクトで Template… に「Page / Site」グループが出ること（Component では出ない）。「ページを追加」を送ると、返答に ```astro path=src/pages/… のブロックが複数入り、提案カードが「プロジェクトの提案 · N ファイル」でファイルのチップを切り替えられること。適用で新しいファイルがツリーに増え、入口ページの Preview に反映されること。存在しないコンポーネントを import した返答は「Cannot resolve …」で auto-fix が走ること。DevTools の Network で `/api/chat`（または direct の場合はプロバイダー）へのリクエスト body に `project.files` が含まれること401 なら「rejected the API key」、429 なら「rate limiting」のバナーになり Retry できること
